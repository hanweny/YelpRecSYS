...
class DualAutoRec:
    ...
    def train(self, 
               ...
               retrain_item = True):
        if self.mode == "DUAL":
            self.train_dual_one_loss(train_y, user_param_ordered_dict, item_param_ordered_dict, val_y, hidden_dim, p_lambda, learning_rate, num_epochs, dropout)
        if self.mode == 'DUAL-SEP':
            self.train_dual_sep(train_y, user_param_ordered_dict, item_param_ordered_dict, val_y, hidden_dim, p_lambda, learning_rate, num_epochs, dropout, retrain_user, retrain_item)
